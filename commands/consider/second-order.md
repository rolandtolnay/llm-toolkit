...
Example chain: Hiring freeze → teams understaffed → leads to → shipping delays → leads to → competitors gain market share → leads to → harder to hire when freeze lifts. A first-order-only analysis would stop at "saves money."
</output_format>

<success_criteria>
- "And then what?" was asked at each level — no effect left without a downstream consequence
- At least two levels of consequences identified for each first-order effect
- Identifies feedback loops and unintended consequences
- Reveals delayed costs or benefits that change the calculus
